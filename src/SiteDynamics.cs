...
namespace Landis.Extension.Succession.BiomassPnET
{
    public class SiteDynamics
    {
        ...
        public static void siteSuccession(SiteCohorts siteCohorts)
        {
            int RDflag;

            float siteRD = SiteVars.SiteRD[siteCohorts.Site];
            double GSO1 = EcoregionPnET.GSO1[siteCohorts.Ecoregion];
            double GSO2 = EcoregionPnET.GSO2[siteCohorts.Ecoregion];
            double GSO3 = EcoregionPnET.GSO3[siteCohorts.Ecoregion];
            double GSO4 = EcoregionPnET.GSO4[siteCohorts.Ecoregion];
        ...
        public static void NaturalMortality(SiteCohorts siteCohorts, int StartAge)
        {
            double DQ_const = 3.1415926 / (4 * 0.0002471 * Math.Pow(PlugIn.ModelCore.CellLength, 2) * Math.Pow(30.48, 2));
            ...
            if (StartAge == 0)
            {
                ...
                foreach (Landis.Library.DensityCohorts.ISpeciesCohorts speciesCohorts in siteCohorts)
                {
                    ...
                    foreach (Landis.Library.DensityCohorts.Cohort cohort in speciesCohorts)
                    {
                        ...
                        {
                            cohort.ChangeTreenumber((int)Math.Pow(PlugIn.ModelCore.CellLength, 2));
                        }
        ...
        public static void Selfthinning(SiteCohorts siteCohorts)
        {
            double targetRD = SiteVars.SiteRD[siteCohorts.Site] - EcoregionPnET.GSO4[siteCohorts.Ecoregion];
        ...
        public static float computeMortalityRD(Landis.Library.DensityCohorts.ICohort cohort, int deadTrees)
        {
            ...
            float deadRD = tmp_term1 * tmp_term2 * tmp_term3 / (float)Math.Pow(PlugIn.ModelCore.CellLength, 2);
            return deadRD;
        }
    }
}
