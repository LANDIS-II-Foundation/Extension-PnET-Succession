    ...
    public class SiteCohorts : ISiteCohorts, Landis.Library.BiomassCohorts.ISiteCohorts, Landis.Library.AgeOnlyCohorts.ISiteCohorts
    {
        ...
        public SiteCohorts(DateTime StartDate, ActiveSite site, ICommunity initialCommunity, bool usingClimateLibrary, string SiteOutputName = null)
        {
            ...
            if (initialSites.ContainsKey(key) && SiteOutputName == null)
            {
               ...
               // this.MaxLAI = initialSites[key].MaxLAI;
        ...
        public bool Grow(List<IEcoregionPnETVariables> data)
        {
            ...
            //Debug
            /*if(PlugIn.ModelCore.CurrentTime == 10 && this.Site.Location.Row == 104 && this.Site.Location.Column == 264)
            {
                ...
                PlugIn.ModelCore.UI.WriteLine("CohortBiomassList = ");
                foreach(double cohortBio in CohortBiomassList)
                {
                    PlugIn.ModelCore.UI.WriteLine(cohortBio.ToString());
                }
            }*/
            ratioAbove10.Clear();
            ...
            for (int m = 0; m < data.Count(); m++ )
            {
                ...
                if (LayeredBins != null)
                {
                    for (int b = LayeredBins.Count() - 1; b >= 0; b--) // main canopy layers
                    {
                        ...
                        foreach (int r in random_range[b]) // sublayers within main canopy b
                        {
                            ...
                            c.Layer = (byte)b;
                            //if (c.Layer > (bins.Count()-1))
                            //    c.Layer = (byte)(bins.Count()-1);
                            //if (c.Layer > c.MaxLayer)
                            //    c.MaxLayer = c.Layer;


                            //float layerLastLAI = CanopyLAI[c.Layer];
                            //float layerMaxLAI = MaxLAI[c.Layer];

                            if (coldKillMonth[spc] == m)
                                coldKillBoolean = true;
                            float O3Effect = lastOzoneEffect[subCanopyIndex - 1];

                            success = c.CalculatePhotosynthesis(subCanopyPrecip, precipCount, leakageFrac, hydrology, mainLayerPAR, ref subcanopypar, O3_ppmh, O3_ppmh_month, subCanopyIndex, SubCanopyCohorts.Count(), ref O3Effect, propRootAboveFrost, subCanopyMelt, coldKillBoolean);
                            //success = c.CalculatePhotosynthesis(subCanopyPrecip, precipCount, leakageFrac, hydrology, mainLayerPAR, ref subcanopypar, O3_ppmh, O3_ppmh_month, subCanopyIndex, SubCanopyCohorts.Count(), ref O3Effect, propRootAboveFrost, subCanopyMelt, coldKillBoolean, layerLastLAI, layerMaxLAI);
                ...
                foreach (Cohort cohort in AllCohorts)
                {
                    ...
                    {
                        PlugIn.ModelCore.UI.WriteLine("DEBUG: Cohort count = " + AllCohorts.Count() + "; CanopyLAISum count = " + CanopyLAISum.Count());
                    }
                }
                for (int layer = 0; layer < MaxCanopyLayers; layer++)
                {
