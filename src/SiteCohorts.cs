...
namespace Landis.Extension.Succession.BiomassPnET
{
    public class SiteCohorts : ISiteCohorts, Landis.Library.BiomassCohorts.ISiteCohorts, Landis.Library.AgeOnlyCohorts.ISiteCohorts
    {
        ...
        public SiteCohorts(DateTime StartDate, ActiveSite site, ICommunity initialCommunity, bool usingClimateLibrary, string SiteOutputName = null)
        {
            ...
            PlugIn.MonthlyPressureHead[site] = new float[0];
            PlugIn.MonthlySoilTemp[site] = new SortedList<float, float>[0];
        ...
        public bool Grow(List<IEcoregionPnETVariables> data)
        {
            ...
            sumPressureHead = 0;
            countPressureHead = 0;
            PlugIn.MonthlyPressureHead[this.Site] = new float[data.Count()];
            PlugIn.MonthlySoilTemp[this.Site] = new SortedList<float, float>[data.Count()];
            for (int m = 0; m < data.Count(); m++ )
            {
                ...
                {
                    float[] snowResults = Permafrost.CalculateSnowDepth(daysOfWinter, snowPack);
                    float sno_dep = snowResults[0];
                    float Psno_kg_m3 = snowResults[1];
                    float damping = Permafrost.CalculateSnowDamping(Psno_kg_m3);
                ...
                if (permafrost)
                {
                    ...
                    float maxDepth = Ecoregion.RootingDepth + Ecoregion.LeakageFrostDepth;
                    float freezeDepth = maxDepth;
                    foreach (var z in depthTempDict.Keys)
                    {
                        float zTemp = depthTempDict[z];
                        if ((zTemp <= 0) && (z < freezeDepth))
                                freezeDepth = z;
                    }
                    ...
                    if (frostFreeSoilDepth < Ecoregion.RootingDepth)
                    {
                        if (lastFrostDepth > 0)
                        {
                            ...
                            Hydrology.FrozenWaterPct = ((Hydrology.FrozenDepth * Hydrology.FrozenWaterPct) + (newFrozenSoil * (hydrology.Water / Ecoregion.RootingDepth))) / (Hydrology.FrozenDepth + newFrozenSoil);
                ...
                if (data[m].Tave > 0)
                {
                    ...
                    PlugIn.MonthlyPressureHead[this.Site][m] = monthlyPressureHead;
                    PlugIn.MonthlySoilTemp[this.Site][m] = monthlyDepthTempDict;
                }
                else
                {
                    PlugIn.MonthlyPressureHead[this.Site][m] = -9999;
                    PlugIn.MonthlySoilTemp[this.Site][m] = null;
                }
                if (PlugIn.ModelCore.CurrentTime > 0)
                {
